...
//            System.out.println("签名：" + certificate.getSignature().toString());
	...
	@Mapping("/jsApiPay")
	public String jsApiPay(@Param(value = "openId", required = false, defaultValue = "o-_-itxuXeGW3O1cxJ7FXNmq8Wf8") String openId) {
		try {
			String timeExpire = DateTimeZoneUtil.dateToTimeZone(System.currentTimeMillis() + 1000 * 60 * 3);
			UnifiedOrderModel unifiedOrderModel = new UnifiedOrderModel()
				.setAppid(wxPayV3Bean.getAppId())
	 ...
	 */
	@Mapping("/tradeBill")
	public String tradeBill(@Param(value = "billDate", required = false) String billDate) {
		try {
	 ...
	 * @return 资金账单下载地址
	 */
	@Mapping("/fundFlowBill")
	public String fundFlowBill(@Param(value = "billDate", required = false) String billDate) {
		try {
			if (StrUtil.isEmpty(billDate)) {
				Calendar calendar = Calendar.getInstance();
				calendar.setTime(new Date());
	...
	public String billDownload(@Param(value = "token") String token,
							   @Param(value = "tarType", required = false) String tarType) {
		try {
	...
	@Mapping("/refund")
	public String refund(@Param(required = false) String transactionId,
						 @Param(required = false) String outTradeNo) {
		try {
